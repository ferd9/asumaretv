    ...
    <div class="span8">
        ...
        if (empty($single)) {
            $this->widget('zii.widgets.CMenu', array(
                ...
                'items' => array(
                    array('label' => Yii::t('yii', 'Latest'), 'url' => array('/meme/default/index', 'action' => 'latest'), 'active' => (Yii::app()->request->url == Yii::app()->homeUrl || Yii::app()->request->url == '/latest')),
                    array('label' => Yii::t('yii', 'Featured'), 'url' => array('/meme/default/index', 'action' => 'featured'), 'visible' => $has_featured_posts),
                    array('label' => Yii::t('yii', 'Trending'), 'url' => array('/meme/default/index', 'action' => 'trending'), 'visible' => $has_trending_posts),
                    array('label' => Yii::t('yii', 'Popular'), 'url' => array('/meme/default/index', 'action' => 'popular')),
                ),
            ));
        }
        ?>
                ...
                <?php if ($top_users): ?>
                    <div id="top-users" class="clearfix">
                        <?php foreach ($top_users as $user): ?>
                            <a class="ttip top-user-avatar" title="<?php echo CHtml::encode($user->nombre . ' ' . $user->apellido_p) ?>" href="<?php echo Yii::app()->createUrl('/meme/default/profile', array('profile' => $user->username)) ?>"><img src="<?php echo Yii::app()->user->getAvatar_url($user->idusuario); ?>" alt="<?php echo CHtml::encode($user->apellido_p . ' ' . $user->apellido_m) ?>" /></a>
                        <?php endforeach ?>
                    </div>
                <?php endif ?>
            </div>
                ...
                <?php if($likes = MemeLike::model()->findAll(array('group' => 't.meme_fk', 'limit' => 10, 'order' => 't.created_at DESC', 'with' => 'meme', 'scopes' => array('visible')))): ?>
                    <div class="clearfix">
                        <?php foreach($likes as $like): ?>
                            <a class="meme-sidebar" href="<?php echo Yii::app()->createUrl('/meme/default/index', array('id' => $like->meme_fk)) ?>"><img src="<?php echo $like->meme->thumb_url ?>" title="<?php CHtml::encode($like->meme->title) ?>" /></a>
                        <?php endforeach ?>
                    </div>
                <?php else: ?>
                        ...
                        <?php if ($remixes = $memes[0]->remixes(array('limit' => 10, 'order' => 'remixes.created_at DESC', 'scopes' => array('visible_remixes')))): ?>
                            <div class="clearfix">
                                <?php foreach ($remixes as $remix): ?>
                                    <a class="meme-sidebar" href="<?php echo Yii::app()->createUrl('/meme/default/index', array('id' => $remix->meme_id)) ?>"><img src="<?php echo $remix->thumb_url ?>" title="<?php CHtml::encode($remix->title) ?>" /></a>
                                <?php endforeach ?>
                            </div>
                        <?php else: ?>
                    ...
                    <div class="corgi_feed_well">
                        ...
                        <div class="sidebar_interior">
                            <div class="clearfix text-center">
                                <a class="meme-sidebar" style="float:none;" href="<?php echo Yii::app()->createUrl('/meme/default/index', array('id' => $remix_of->meme_id)) ?>"><img style="width: auto;" src="<?php echo $remix_of->thumb_url ?>" title="<?php CHtml::encode($remix_of->title) ?>" /></a>
                            </div>
                        </div>
                    </div>
                <?php Yii::app()->plugin->endBlock() ?>
